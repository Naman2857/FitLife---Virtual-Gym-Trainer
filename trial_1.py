...
actions = np.array(['B-Curls', '', 'Neutral'])
model = load_model('gym.h5')

# colors = [(245, 117, 16), (117, 245, 16), (16, 117, 245)]
colors = [(245, 117, 16), (251,248,230), (16, 117, 245)]
...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    while cap.isOpened():
                ...
                if res[np.argmax(res)] > threshold:
                    try:
                        ...
                        if actions[np.argmax(res)] == "B-Curls" or actions[np.argmax(res)] == "Neutral":
                            if angle > 167.24:
                                stage_c = "down"
                            if angle < 11.88 and stage_c == 'down':
                                stage_c = "up"
                                curls += 1
                                print(f"curls: {curls}")
                                # cv2.putText(image, "      correct posture", (3, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
        ...
        # cv2.putText(image, f"      correct posture", (3, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
